...
export default function TabFuncionarios() {
  ...
  const [modal, setModal] = useState(null) // null | 'novo' | objeto funcionario
  ...
  const [pausaForm, setPausaForm] = useState({ inicio: '', fim: '', motivo: 'Ferias' })
  ...
  async function pausar(f) {
    if (!pausaForm.inicio) { alert('Informe a data de inicio.'); return }
    if (pausaForm.fim && pausaForm.fim < pausaForm.inicio) { alert('A data final nao pode ser menor que a inicial.'); return }

    const pausa = {
      id: Date.now().toString(36),
      inicio: pausaForm.inicio,
      fim: pausaForm.fim || null,
      motivo: pausaForm.motivo || 'Ferias'
    }
    ...
    setModalPausa(null)
    setPausaForm({ inicio: '', fim: '', motivo: 'Ferias' })
    carregar()
  }
  ...
  function abrirDemissao(f) {
    if (f.is_owner) {
      alert('Este funcionario esta marcado como dono/socio e nao pode ser demitido.')
      return
    }
    setModalDemissao(f)
  ...
  async function confirmarDemissao() {
    ...
    if (modalDemissao.is_owner) { alert('Este funcionario esta marcado como dono/socio e nao pode ser demitido.'); return }
    if (!demissaoForm.data_saida) { alert('Informe a data de demissao.'); return }
  ...
  const tituloLista = { ativos: 'Funcionarios ativos', pausados: 'Funcionarios pausados', demitidos: 'Funcionarios demitidos' }[filtroStatus]
  const vazioLista = { ativos: 'Nenhum funcionario ativo encontrado.', pausados: 'Nenhum funcionario pausado encontrado.', demitidos: 'Nenhum funcionario demitido encontrado.' }[filtroStatus]
    ...
    <div>
      ...
      {modalPausa && (
        <div style={overlay}>
          <div style={{ background: '#fff', borderRadius: 12, padding: 24, width: 340 }}>
            <div style={{ fontWeight: 700, marginBottom: 16 }}>⏸️ Pausar {modalPausa.nome.split(' ')[0]}</div>
            <label style={lbl}>Data de inicio</label>
            ...
            <input style={inp} placeholder="Ex: Ferias, Licenca..." value={pausaForm.motivo} onChange={e => setPausaForm(p => ({ ...p, motivo: e.target.value }))} />
        ...
        <div style={overlay}>
          <div style={{ background: '#fff', borderRadius: 12, padding: 24, width: 360 }}>
            ...
            <div style={{ color: '#666', fontSize: 13, marginBottom: 16 }}>
              A partir desta data, o funcionario sai do quadro ativo e nao participa de novas comissoes nem zonas.
            </div>
            <label style={lbl}>Data de demissao</label>
            <input
              style={inp}
              type="date"
            ...
            />
            <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
              <button onClick={confirmarDemissao} style={btnRed}>Confirmar demissao</button>
        ...
        <div style={overlay}>
          <div style={{ background: '#fff', borderRadius: 16, padding: 24, width: '100%', maxWidth: 500, maxHeight: '90vh', overflowY: 'auto' }}>
            <div style={{ fontWeight: 700, fontSize: 16, marginBottom: 20 }}>
              {modal === 'novo' ? '➕ Novo funcionário' : `✏️ Editar — ${modal.nome?.split(' ')[0]}`}
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              ...
              <label style={{ gridColumn: '1/-1', display: 'flex', alignItems: 'center', gap: 8, fontSize: 14, cursor: 'pointer' }}>
                ...
                Nao participa do rateio de comissao (dono/socio)
              </label>
            </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '180px 1fr', gap: 16, alignItems: 'start' }}>
        <div style={{ background: '#fff', borderRadius: 12, padding: 12, border: '1px solid #e5e7eb', position: 'sticky', top: 12 }}>
          <div style={{ fontSize: 12, color: '#888', fontWeight: 700, margin: '2px 8px 10px' }}>Funcionarios</div>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 8 }}>
                <div>
                  ...
                  <div style={{ color: '#666', fontSize: 12, marginTop: 2 }}>
                    ...
                    {f.data_admissao && ` � Admissao: ${new Date(f.data_admissao + 'T12:00').toLocaleDateString('pt-BR')}`}
                  </div>
                  {f.data_nascimento && (
                  ...
                  {statusVisual(f) === 'demitido' && (
                    <div style={{ color: '#dc2626', fontSize: 12, marginTop: 4, fontWeight: 600 }}>
                      Demissao: {f.data_saida ? new Date(f.data_saida + 'T12:00').toLocaleDateString('pt-BR') : 'sem data informada'}
                    </div>
                  )}
                ...
                {statusVisual(f) !== 'demitido' && (
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                    ...
                    {!f.is_owner && f.status === 'ativo' && <button onClick={() => { setModalPausa(f); setPausaForm({ inicio: '', fim: '', motivo: 'Ferias' }) }} style={btnAcao}>Pausar</button>}
